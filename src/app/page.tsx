    ...
    <ToastSystem maxToasts={3} placement="bottom-right">
      <main className="min-h-screen p-8 bg-gray-50 space-y-12">
        ...
        <section className="space-y-4">
          ...
          {/* 전역 Toast 시스템 테스트 */}
          <div className="mb-8">
            <h3 className="heading2 mb-4">🌐 전역 Toast 시스템 (새로운 방식)</h3>
            <div className="flex gap-4 flex-wrap">
              <button
                onClick={() => toast.success("요청하신 인증번호를 재발송하였습니다. \n 1 저장 완료, 0 실패")}
                className="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600"
              >
                Success Toast
              </button>
              <button
                onClick={() => toast.error("저장에 실패했습니다.", "네트워크 연결을 확인해주세요.")}
                className="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600"
              >
                Error Toast
              </button>
              <button
                onClick={() => toast.warning("주의가 필요합니다.", "이 작업은 되돌릴 수 없습니다.")}
          ...
          {/* 기존 방식 테스트 */}
          <div>
            <h3 className="heading2 mb-4">🔧 기존 방식 (단일 토스트)</h3>
            <div className="flex gap-4 flex-wrap">
              <button
